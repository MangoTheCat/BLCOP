...
stocks, 2 of which are technology stocks and the other 4 of which are from the financial sector. Initially, she believes that the average of the 2 tech stocks will outperform one of the financial stocks, say $\frac{1}{2}( \textrm{DELL} + \textrm{IBM}) - \textrm{MS} \sim N(0.06,  0.01)$.  
We will create a **BLViews** class object with the `BLViews()`
...
than manually computing these, it is convenient to use the `BLPosterior()` wrapper function.  It will compute these "alphas", as well
as the variance-covariance matrix of a returns series, and will then call `poseriorEst()` automatically.
...
```{r, echo = TRUE, print = TRUE, fig = TRUE}
