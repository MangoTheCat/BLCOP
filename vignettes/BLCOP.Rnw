...
\section{Using the Black-Litterman model in BLCOP}
...
We will create a \code{BLViews} class object with the \texttt{BLViews}
...
\cite{MeucciCOP}, p.9.  Suppose that we wish to invest in 4 market indices (S\&P500, FTSE, CAC and DAX). Meucci suggests a 
